...
let exo1li = Array.from(document.querySelector('ul').children);
exo1li.forEach(element => {
    // element.addEventListener('click', sayMyName);
    element.addEventListener('dblclick' , changeName)
})
...
function changeName() {
    this.innerHTML = prompt('new name?')
}
